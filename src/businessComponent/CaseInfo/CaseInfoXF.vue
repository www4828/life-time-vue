  ...
  <div class="caseInfo">
    <div class="caseHeader">
      <div class="flexWrapper">
        <div class="taskId">
          <span class="titleLabel">信访单号</span>
          <span
            class="titleValue cursor marginR"
            @click="jumpToXFDetail(props.caseInfo.taskId + '')"
            >{{ props.caseInfo.taskId }}</span
          >
        </div>
        <CaseIcon :icons="markIcon" :task-id="props.caseInfo.taskId + ''" />
      </div>
      <div class="concatWrapper">
        <span class="titleLabel" v-show="props.caseInfo.xfTaskId">工单编号</span>
        <span
          class="titleValue cursor marginR"
          @click="jumpToDetail(props.caseInfo.xfTaskId + '')"
          >{{ props.caseInfo.xfTaskId }}</span
        >
        <div class="div-flex-center" v-show="caseInfo.externalNumber !== ''">
          <span class="titleLabel">关联单号</span>
          <span
            class="titleValue concat"
            v-show="caseInfo.xfTaskId !== ''"
            @click="
              jump(props.caseInfo.eventSourceCode === '0', props.caseInfo.externalNumber)
            "
          ...
          <span
            class="titleValue concat"
            v-show="caseInfo.xfTaskId === ''"
            @click="canJump(props.caseInfo.externalNumber)"
            >{{ props.caseInfo.externalNumber || "-" }}</span
          >
        </div>
    ...
    <div class="caseContent">
      <div class="caseDetail">
          ...
          <el-col :span="10">
            ...
            <span class="value address">
              <el-tooltip
                placement="top"
                effect="light"
                :content="props.caseInfo.sourceAddress"
        ...
        <el-row :gutter="20" class="marginB">
          ...
          <div class="caseMarkWrapper" :class="{ topNegative: isShortDes }">
...
import { Icon } from "@/businessComponent/CaseIcon/type";
...
const { markIcons } = useMarkIconXF(props.caseInfo.taskId + "");
const markIcon = ref<Icon[]>([]);
...
const canJump = async (externalNumber: string) => {
  if (externalNumber !== "") {
...
watchEffect(() => {
  if (props.caseInfo.tdjzTime !== "") {
    markIcon.value = [
      ...markIcons.icons,
      {
        name: "",
        icon: "",
        note: "退单截止时间" + props.caseInfo.tdjzTime,
        remark: "退单截止时间" + props.caseInfo.tdjzTime,
      },
    ];
  } else {
    markIcon.value = markIcons.icons;
  }
});
