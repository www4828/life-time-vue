<template>
  <Title align="flex-start">
    <template #default>
      <span :class="['title_tag', isActive == 0 ? 'active' : '']" @click="changeTag(0)">复核流程</span>
      <span :class="['title_tag', isActive == 1 ? 'active' : '']" @click="changeTag(1)">处理流程</span>
      <span :class="['title_tag', isActive == 2 ? 'active' : '']" @click="changeTag(2)">流程比对</span>
      <img src="@/assets/process/sortIconDown.png" class="sort" v-if="isSort" @click="turnOver" alt="" />
      <img src="@/assets/process/sortIcon.png" class="sort" v-else @click="turnOver" alt="" />
    </template>
    <template #content>
      <div v-if="isActive === 2">
          ...
          <div class="content-wrap">
            ...
            <div class="content" v-for="process in node.child">
              <div class="contentWrapper">
                <div class="wrapper" @click="showDetail(process)"
                  :class="[process.taskId === props.taskId ? 'review' : 'ywd']">
                ...
                <div class="files">
                  <div class="pic" v-if="process.caseAttachmentList.filter(({ fileName }) =>
        fileName.match(/\.(jpe?g|png|gif)/i)
      ).length > 0
        ">
                    <span>案件图片:</span>
                    <el-image style="width: 100px; height: 100px; margin-right: 10px" :src="picture" :zoom-rate="1.2"
                      v-for="(picture, index) in process.caseAttachmentList
        .filter(({ fileName }) =>
          fileName.match(/\.(jpe?g|png|gif)/i)
        )
        .map(({ url }) => url)" :preview-src-list="process.caseAttachmentList
          .filter(({ fileName }) =>
            fileName.match(/\.(jpe?g|png|gif)/i)
          )
          .map(({ url }) => url)
        " :initial-index="activeIndex" @click="setIndex(index)" fit="cover" />
                  </div>
                  <div class="pic" v-if="process.caseAttachmentList.filter(({ fileName }) =>
        ...
        ">
                    <span>案件文件:</span>
                    <el-link :href="file.url" target="_blank" class="margin10"
                      v-for="file in getFiles(process.caseAttachmentList)">{{ file.fileName }}</el-link>
                  </div>
                </div>
              </div>
            </div>
      ...
      <StepDetail :process="process" :dialogVisible="dialogVisible" @close="dialogVisible = false" />
    </template>
  </Title>
</template>
...
const getCompareRecord = () => {
  lettersVisitService.getCompareRecord({ reviewTaskId: props.taskId, taskId: props.reviewTaskId }).then((res: Response) => {
    res.data.forEach((item: any) => {
