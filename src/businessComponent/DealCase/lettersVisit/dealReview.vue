<template>
  <div>
    <Title title="办理结果">
      <template #content>
        <DoneForm ref="doneForm" label="办理描述" :form="state.form" nodeCode="XC201-1" />
      </template>
    </Title>
    <ContactFirst :is-choice="state.canChoice" :taskId="props.flowModel.taskId" :is-only-look="true" type="先联"
      @get-result="getResult"></ContactFirst>
    <Title title="办结报告" v-if="is12345">
      <template #content>
        <DoneReport ref="doneReport" :case-info="props.caseInfo" :taskId="props.taskId" :node-code="['XC201-2','XC201-3','XC201-4','XC201-5']" />
      </template>
    </Title>
    <Title title="办理复核">
      <template #content>
...
const save = () => {
  ...
  // params.caseDisposeDto.taskId = props.taskId
  if (is12345.value) {
