<template>
  <div class="dispatchWrapper">
    <div class="content">
      <Query @getPrams="getParams" :node-code="props.nodeCode" export />
  ...
  caseService
    ...
    .then(async (res) => {
      if (res.code === 200 && res.data.results?.length > 0) {
        ...
        let taskIdList = [] as Array<string>
        caseInfos.value.forEach((item: any) => {
          taskIdList.push(item.taskId)
          if(item.caseInfo.emergency === '2' && item.caseInfo.eventSourceCode === '1'  && props.feedBack){
            item.buttonInfo.push({
              name: '次紧急反馈',
              id: 'C219',
              code: 'C219'
            })
...
if (route.path === "/workTicket/dispatch") {
  timer.value = setInterval(getAll, 1000 * 60 * 5);
}
</script>
