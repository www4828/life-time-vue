        ...
        >
          ...
          <template v-if="isActive(v)">
            ...
            <SvgIcon
              ...
              v-if="!v.meta.isAffix"
              @click.stop="
                closeCurrentTagsView(getThemeConfig.isTagsView ? v.path : v.url)
              "
            />
          </template>
          <SvgIcon
            ...
            v-if="!v.meta.isAffix"
            @click.stop="closeCurrentTagsView(getThemeConfig.isTagsView ? v.path : v.url)"
          />
        </li>
      </ul>
    </el-scrollbar>
...
import Contextmenu from "@/layout/components/defaults/contextmenu.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const openCurrenFullscreen = async (path: string) => {
      ...
      store.commit("themeConfig/TOGGLE_TAGS");
    };
    ...
    const setTagsViewHighlight = (v: any) => {
      ...
      if (v.meta.isDynamic) {
        return v.meta.isDynamicPath + "-" + path;
      } else {
        return v.path;
      }
    ...
    watch(
      () => getThemeConfig.value.isSortableTagsView,
      (val) => {
        initSortable();
      },
      {
