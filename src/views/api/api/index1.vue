  ...
  <Layout>
    <template #tree>
      <div class="tree_container">
        ...
        <div class="tree">
          <ApiGroupTree @handleNodeClick="handleNodeClick" @addHandle="addApiGroupHandle"
            @editHandle="editApiGroupHandle" @getTree="val => state.apiGroupList = val" :showSearch="false"
            :showLine="false" :treeJson="{ type: apiGroupSever }" :refresh="state.refresh"
            @setRefresh="() => state.refresh = false" operate />
        </div>
      </div>
      <el-dialog v-model="state.groupDialogShow" :title="state.groupFlag === 'add' ? '创建分组' : '修改分组'" width="500">
        <el-form ref="groupForm" :inline="true" :model="state.groupForm" class="dataForm" :rules="rules">
          <el-form-item label="分组编码:" prop="groupCode" class="long_item">
            <el-input v-model="state.groupForm.groupCode" placeholder="接口编码" :disabled="state.groupFlag === 'edit'"
              clearable />
          </el-form-item>
          ...
          <el-form-item label="上级分组:" prop="parentCode" class="long_item" v-if="state.groupFlag === 'add'">
            <el-tree-select v-model="state.groupForm.parentCode" :data="state.apiGroupList" :render-after-expand="false"
              node-key="code" clearable check-strictly :props="{ children: 'child', label: 'name', value: 'code' }" />
          </el-form-item>
        </el-form>
        <template #footer>
      ...
      <div class="sh3h-search-box div-flex-column">
        <el-row>
          <el-col :span="5">
            ...
            <el-select class="sh3h-search-input" v-model="apiState.apiSqlInfo.dataSourceId" placeholder="DB"
              @change="getSchema">
          ...
          <el-col :span="5">
            ...
            <el-select class="sh3h-search-input" v-model="apiState.apiSqlInfo.schemaName" placeholder="Schema"
              @change="getTable">
          ...
          <el-col :span="5">
            ...
            <el-select class="sh3h-search-input" v-model="apiState.apiSqlInfo.operaType" placeholder="sql"
    ...
    <template #content>
      <div class="code-mirror">
        <Codemirror v-model="apiState.apiSqlInfo.sqlScript" :extensions="extensions" :autofocus="true"
          :indent-with-tab="true" :tabSize="2" />
      </div>
      <div class="result-wrap">
        <el-tabs v-model="apiState.activeName" class="demo-tabs" @tab-click="tabChange">
        ...
        <div class="result-content">
          <v-ace-editor v-if="apiState.activeName === 'first' && apiState.jsonValue" v-model:value="apiState.jsonValue"
            @init="jsonFormat" :lang="aceConfig.lang" :theme="aceConfig.theme" :options="aceConfig.options"
            :readonly="aceConfig.readOnly" class="ace-editor" />
          <el-empty description="暂无数据" v-if="apiState.activeName === 'first' && !apiState.jsonValue" />
          <BaseInfo :list='state.apiGroupList' v-show="apiState.activeName === 'second'" ref="baseInfoRef"
            :formData="state.apiInfo.apiBaseInfo" />
...
const groupForm = ref()
const { types } = useCode()
const { apiState, getSchema, getTable, aceConfig, checkSQLSentence, checkRunSQL, sqlFormat, runSQL, jsonFormat, changeOperaType } = useApiData()
